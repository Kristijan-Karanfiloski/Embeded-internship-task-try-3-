...
const cachedCards = localStorage.getItem(cardsKey);
if (cachedCards) {
  console.log("Retrieved cards data from cache");
} else {
  // retrieve cards data from server and cache it
  fetch(cardsUrl)
    .then((response) => response.json())
    .then((cards) => {
      localStorage.setItem(cardsKey, JSON.stringify(cards));
      console.log("Retrieved cards data from server and cached it");
      loadInitialCards();
      loadButton.addEventListener("click", loadData);
    })
    .catch((error) => {
      console.error("Error retrieving cards data:", error);
    });
}
...
function loadData() {
  ...
  if (document.querySelectorAll(".card").length === cards.length) {
    loadButton.style.display = "none"; // hide loadMoreButton if all cards are loaded
  }
}
